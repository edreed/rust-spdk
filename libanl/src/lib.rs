...
/// A flag passed to the `mode` argument of [`getaddrinfo_a`] to perform lookups synchronously.
pub const GAI_WAIT: c_int = 0;

/// A flag passed to to the `mode` argument of [`getaddrinfo_a`] to perform lookups asynchronously.
...
pub struct Error(
    /// An error code returned by the [`getaddrinfo`] family of functions.
    ///
    /// [`getaddrinfo`]: https://www.man7.org/linux/man-pages/man3/getaddrinfo.3.html
    pub c_int,
);
