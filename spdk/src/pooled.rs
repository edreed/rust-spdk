...
use std::{
    ffi::CStr,
    mem::{self, offset_of, size_of_val, ManuallyDrop, MaybeUninit},
    ops::{Deref, DerefMut},
    pin::Pin,
    process::abort,
    ptr::{self, addr_of_mut, drop_in_place, NonNull},
    sync::atomic::{AtomicUsize, Ordering},
};

use spdk_sys::{
    spdk_mempool, spdk_mempool_count, spdk_mempool_create, spdk_mempool_free,
    spdk_mempool_from_obj, spdk_mempool_get, spdk_mempool_get_name, spdk_mempool_put,
    SPDK_ENV_SOCKET_ID_ANY, SPDK_MEMPOOL_DEFAULT_CACHE_SIZE,
};

use crate::errors::{Errno, EINVAL, ENOMEM};
...
impl MemPool {
    ...
    pub fn name(&self) -> &CStr {
        unsafe { CStr::from_ptr(spdk_mempool_get_name(self.pool.as_ptr())) }
    }
    ...
    pub fn available(&self) -> usize {
        unsafe { spdk_mempool_count(self.pool.as_ptr()) }
    }
...
impl<T> ArcPooled<T> {
    ...
    pub fn try_new(data: T, pool: &MemPool) -> Result<Self, Errno> {
        ...
        Ok(Self {
            inner: unsafe { NonNull::new_unchecked(Pooled::into_raw(inner)) },
        })
    }
        ...
        F: FnOnce(&WeakPooled<T>) -> T,
    {
        ...
        let mut inner: NonNull<ArcPooledInner<T>> =
            unsafe { NonNull::new_unchecked(Pooled::into_raw(uninit)).cast() };

        let weak = WeakPooled { inner };
        ...
        loop {
            ...
            match inner.strong.compare_exchange_weak(
                ...
                Ok(old_count) => {
                    ...
                    return Some(ArcPooled { inner: self.inner });
                }
