...
use spdk_sys::{
    spdk_mempool, spdk_mempool_count, spdk_mempool_create_ctor, spdk_mempool_free,
    spdk_mempool_from_obj, spdk_mempool_get, spdk_mempool_get_name, spdk_mempool_obj_iter,
    spdk_mempool_put, SPDK_ENV_SOCKET_ID_ANY,
};
...
where
    T: Default,
{
    unsafe extern "C" fn initialize_object(
        _mempool: *mut spdk_mempool,
        _ctx: *mut c_void,
        obj: *mut c_void,
        _obj_idx: u32,
    ) {
    ...
    pub fn name(&self) -> &CStr {
        unsafe { CStr::from_ptr(spdk_mempool_get_name(self.0.as_ptr())) }
    }
    ...
    pub fn available(&self) -> usize {
        unsafe { spdk_mempool_count(self.0.as_ptr()) }
    }

    /// Gets an object from the pool.
    pub fn get(&self) -> Option<Pooled<T>> {
        NonNull::new(unsafe { spdk_mempool_get(self.0.as_ptr()) as *mut T }).map(Pooled)
    }

    /// Gets an object from the pool as a pinned smart pointer.
    pub fn get_pinned(&self) -> Option<Pin<Pooled<T>>> {
        self.get()
            .map(|pooled| unsafe { Pin::new_unchecked(pooled) })
    }
}
    ...
    T: Default,
{
    ...
    where
        F: FnMut(&T),
    {
        unsafe extern "C" fn callback<T, F>(
            _mp: *mut spdk_mempool,
            f: *mut c_void,
            obj: *mut c_void,
            _obj_idx: u32,
        ) where
        ...
        unsafe {
            spdk_mempool_obj_iter(
                self.0.as_ptr(),
                Some(callback::<T, F>),
                for_each_fn.as_mut() as *mut _ as *mut c_void,
            )
        }
    }
}
    ...
    fn drop(&mut self) {
        unsafe {
            *self.0.as_mut() = Default::default();
            ...
            spdk_mempool_put(
                spdk_mempool_from_obj(self.0.as_ptr().cast()),
                self.0.as_ptr().cast(),
            );
        }
    }
...
impl<T> Unpin for Pooled<T> where T: Default {}
